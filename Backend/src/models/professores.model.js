...
const getAllProfessors = async () =>
  db.collection("professores").find().toArray();
...
const getAllAvaliacoes = async () =>
  db.collection("avaliacoes").find().toArray();
...
const enviarEmailAutenticacao = async (id, userEmail) => {
  const token = jwt.sign({ userId: id, verificarEmail: true }, segredo, {
    expiresIn: "1h",
  });
...
const iniciarRedefinicaoSenha = async (email) => {
  try {
    ...
    const token = jwt.sign({ userId: id, redefinirSenha: true }, segredo, {
      expiresIn: "1h",
    });
...
const redefinirSenha = async (novaSenha) => {
  try {
    ...
    const result = await db
      .collection("usuarios")
      .updateMany({}, { $set: { senha: senhaHash } });
